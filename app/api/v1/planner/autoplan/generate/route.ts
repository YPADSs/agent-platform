import { NextResponse } from 'next/server';
import { generateAutoplan } from '@/lib/autoplan';
import { getPrisma } from '@/lib/prisma';
import { parseIsoDateOnly, requirePlannerAccess, startOfUtcDay } from '@/lib/planner';
import { requireSession } from '@/lib/session';

type GenerateBody = {
  weekStart?: unknown;
};

async function getAuthenticatedUser() {
  const session = await requireSession();
  const email = session.user?.email;

  if (!email) {
    const err = new Error('UNAUTHENTICATED') as Error & { status?: number };
    err.status = 401;
    throw err;
  }

  const prisma = getPrisma();
  const user = await prisma.user.findUnique({
    where: { email },
    select: { id: true },
  });

  if (!user) {
    const err = new Error('USER_NOT_FOUND') as Error & { status?: number };
    err.status = 404;
    throw err;
  }

  return user;
}

export async function POST(req: Request) {
  try {
    const body: GenerateBody = await req.json().catch(() => ({}));
    const requestedWeekStart =
      typeof body.weekStart === 'string' ? parseIsoDateOnly(body.weekStart) : new Date();

    if (!requestedWeekStart) {
      return NextResponse.json({ error: 'INVALID_WEEK_START' }, { status: 422 });
    }

    const user = await getAuthenticatedUser();
    await requirePlannerAccess(user.id);

    const plan = await generateAutoplan(user.id, startOfUtcDay(requestedWeekStart));
    return NextResponse.json({ plan });
  } catch (error) {
    const status = (error as { status?: number })?.status ?? 401;
    const code =
      status === 403
        ? 'PREMIUM_REQUIRED'
        : status === 404
        ? 'USER_NOT_FOUND'
        : 'UNAUTHENTICATED';

    return NextResponse.json({ error: code }, { status });
  }
}
